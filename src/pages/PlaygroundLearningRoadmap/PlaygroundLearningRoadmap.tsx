...
import {
    Map, CheckCircle, Circle, Lock, ChevronDown, ChevronUp,
    Shield, Globe, Terminal, Cpu, Lock as LockIcon, Eye, Trophy, Star
} from 'lucide-react';
        ...
        <div className="pt-32 pb-20">
            <div className="container-custom">
                ...
                <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                    <div className="w-20 h-1 bg-gradient-to-r from-emerald-500 to-teal-600 mx-auto mb-8 rounded-full" />
                    ...
                    <h1 className="text-5xl lg:text-6xl font-bold mb-6">
                        ...
                        <span className="bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">Roadmap</span>
                    </h1>
                ...
                <motion.div className="flex flex-wrap justify-center gap-6 mb-12"
                    ...
                    {[
                        ...
                        <div key={label} className="flex items-center gap-3 px-5 py-3 rounded-xl border border-white/5 bg-white/[0.02]">
                            <Icon className="w-5 h-5 text-emerald-400" />
                            <div><span className="text-lg font-bold text-white">{value}</span><span className="text-xs text-gray-500 ml-1">{label}</span></div>
                        </div>
                    ))}
                </motion.div>
                ...
                <div className="flex gap-2 mb-8">
                    {['All', 'Beginner', 'Intermediate', 'Advanced'].map(f => (
                        <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${filter === f ? 'bg-emerald-500 text-white' : 'bg-white/[0.05] text-gray-400 hover:text-white hover:bg-white/10'}`}>{f}</button>
                    ))}
                </div>
                ...
                <motion.div className="space-y-4" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                    {filtered.map((path) => {
                        ...
                        return (
                            <motion.div key={path.id} variants={staggerItem} className={`rounded-xl border ${path.borderColor} ${path.bgColor} overflow-hidden`}>
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`p-3 rounded-xl border ${path.borderColor} bg-white/[0.04]`}>
                                            <Icon className={`w-6 h-6 ${path.iconColor}`} />
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                <h3 className="text-lg font-bold text-white">{path.title}</h3>
                                                <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${levelColor[path.level]}`}>{path.level}</span>
                                            </div>
                                    ...
                                    {isOpen && (
                                        ...
                                        >
                                            <div className="px-6 pb-6">
                                                <p className="text-gray-400 text-sm mb-5 pb-5 border-b border-white/5">{path.description}</p>
                                                <div className="space-y-3">
                                                    {path.steps.map((step, i) => (
                                                        <div key={i} className="flex gap-4 group">
                                                            <div className="flex flex-col items-center">
                                                                <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${step.done ? 'bg-emerald-500 border-emerald-500' : `border-white/10 bg-white/[0.03]`}`}>
                                                                    {step.done ? <CheckCircle className="w-4 h-4 text-white" /> : <span className={`text-xs font-bold ${path.iconColor}`}>{i + 1}</span>}
                                                                </div>
                                                                {i < path.steps.length - 1 && <div className="w-px flex-1 mt-1 mb-1 bg-white/5" />}
                                                            </div>
