...
const addOnServices = [
    { id: 'addon-domain', icon: Globe, name: 'Domain & Hosting', price: 500000, priceLabel: 'Rp 500,000/year', desc: 'Domain .com + Hosting 10GB SSD', gradient: 'from-cyan-500 to-blue-500' },
    { id: 'addon-db', icon: Database, name: 'Database Management', price: 2000000, priceLabel: 'Rp 2,000,000', desc: 'Database setup & optimization', gradient: 'from-green-500 to-emerald-500' },
    { id: 'addon-support', icon: Headphones, name: 'Priority Support 24/7', price: 3000000, priceLabel: 'Rp 3,000,000/month', desc: 'Dedicated support engineer', gradient: 'from-purple-500 to-violet-500' },
    { id: 'addon-source', icon: FileCode, name: 'Source Code & Docs', price: 1500000, priceLabel: 'Rp 1,500,000', desc: 'Full source code + technical docs', gradient: 'from-orange-500 to-red-500' },
];
...
function AddToCartButton({ pkg }: { pkg: ServicePackage }) {
    ...
    return (
        <motion.button
            onClick={handleAdd}
            whileTap={{ scale: 0.95 }}
            className={`w-full py-3.5 rounded-xl font-semibold text-sm transition-all duration-300 ${justAdded
...
function AddOnButton({ addon }: { addon: typeof addOnServices[0] }) {
    ...
    return (
        <motion.button
            onClick={handleAdd}
            whileTap={{ scale: 0.95 }}
            className={`mt-3 w-full py-2 rounded-lg text-xs font-semibold transition-all duration-300 ${justAdded
    ...
    return (
        <div className="pt-32 pb-20">
            <div className="container-custom">
                ...
                >
                    <motion.div
                        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-orange-500/30 bg-orange-500/10 mb-8"
                    ...
                    <div className="w-20 h-1 bg-gradient-to-r from-primary via-secondary to-primary mx-auto mb-8 rounded-full" />

                    <div className="inline-flex p-4 rounded-xl border border-primary/20 bg-primary/5 mb-6">
                    ...
                    <div className="flex flex-wrap gap-4 justify-center">
                        ...
                        {totalItems > 0 && (
                            <Link
                                ...
                                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-primary/30 bg-primary/5 text-primary font-semibold transition-all hover:bg-primary/10"
                            >
                ...
                >
                    {[
                        ...
                        <motion.div
                            key={idx}
                            className="relative rounded-xl p-6 text-center border border-white/5 hover:border-primary/20 transition-all duration-300 bg-white/[0.02] hover:bg-white/[0.05] group"
                            variants={staggerItem}
                            whileHover={{ y: -4 }}
                        >
                            <stat.icon className="w-6 h-6 text-primary mx-auto mb-2 opacity-60" />
                            <div className="text-3xl lg:text-4xl font-bold bg-gradient-to-br from-white to-gray-400 bg-clip-text text-transparent mb-1">
                                {stat.value}
                            </div>
                            <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">
                                {stat.label}
                            </div>
                            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-[2px] bg-gradient-to-r from-primary to-secondary group-hover:w-3/4 transition-all duration-500" />
                        </motion.div>
                    ))}
                </motion.div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                            {servicePackages.map((pkg, idx) => {
                                ...
                                return (
                                    <motion.div
                                        key={pkg.id}
                                        className={`relative rounded-2xl border transition-all duration-500 group overflow-hidden h-full flex flex-col ${pkg.popular
                                    ...
                                    >
                                        ...
                                        <div className="absolute top-4 right-4 z-10">
                                            <motion.div
                                                className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-gradient-to-r from-red-500 to-orange-500 shadow-lg shadow-red-500/30"
                                        ...
                                        <div className="p-6 lg:p-8 flex flex-col flex-1">
                                            ...
                                            <div className="mb-6 pb-6 border-b border-white/5 h-[6rem] flex flex-col justify-center">
                                                ...
                                                <div className="flex items-baseline gap-1">
                                                    <span className={`text-2xl lg:text-3xl font-extrabold bg-gradient-to-r ${pkg.gradient} bg-clip-text text-transparent whitespace-nowrap`}>
                                            ...
                                            <Link
                                                to={pkg.link}
                                                className="mt-3 flex items-center justify-center gap-1.5 text-xs text-gray-500 hover:text-primary transition-colors"
                                            >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {addOnServices.map((addon, idx) => {
                            ...
                            return (
                                <motion.div
                                    key={addon.id}
                                    className="relative rounded-xl p-5 border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-300 group h-full flex flex-col"
                                ...
                                >
                                    <div className="flex items-start gap-3 flex-1">
                                        <div className={`p-2 rounded-lg bg-gradient-to-br ${addon.gradient} flex-shrink-0`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <span className="text-primary font-bold text-sm">{addon.priceLabel}</span>
                                        </div>
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {processSteps.map((step, idx) => {
                            ...
                            return (
                                <motion.div
                                    key={idx}
                                    className="relative rounded-xl p-6 border border-white/5 bg-white/[0.02] text-center group hover:bg-white/[0.04] transition-all duration-300"
                                    initial={{ opacity: 0, y: 20 }}
                                    whileInView={{ opacity: 1, y: 0 }}
                                    viewport={{ once: true }}
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        <div className="inline-flex p-3 rounded-xl bg-primary/5 border border-primary/10 mb-4">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {CERTIFICATIONS.map((cert, idx) => (
                                    <motion.div
                                        key={idx}
                                        className="rounded-xl p-4 text-center border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-all duration-300"
                ...
                {/* CTA Section */}
                <motion.div
                    className="relative border border-white/5 rounded-2xl p-12 lg:p-16 text-center bg-gradient-to-b from-white/[0.02] to-transparent overflow-hidden"
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                >
                    ...
                    <div className="relative z-10 space-y-8">
                        <motion.div
                            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-orange-500/20 bg-orange-500/5"
                            initial={{ scale: 0.8 }}
                            whileInView={{ scale: 1 }}
                            viewport={{ once: true }}
                        ...
                        >
                            ...
                            <span className="text-xs font-semibold text-orange-300 uppercase tracking-widest">Limited Time Offer</span>
                        </motion.div>
                        ...
                        <div className="flex flex-wrap gap-4 justify-center">
                            <Link
                                ...
                                className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-white font-semibold shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:scale-[1.02] transition-all"
                            >
                                <ShoppingCart className="w-5 h-5" />
